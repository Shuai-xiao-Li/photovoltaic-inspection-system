/**
 * @file key_control.c
 * @brief 开发板按键GPIO交互逻辑，解析KEY0/KEY1的短按/长按操作，实现脱机动作选择和本地交互调参。
 * @author 李帅 赵禹博 吴坨鑫
 * @date 6月12号
...
static const char *action_name(KeyAction_t action)
{
    switch (action)
    {
        ...
        case KEY_ACTION_TEST_HEADING: return "TEST HEAD";
        case KEY_ACTION_TEST_AUTO:    return "TEST AUTO";
        case KEY_ACTION_ZERO_YAW:     return "ZERO YAW";
        case KEY_ACTION_SAVE_PARAMS:  return "SAVE";
        ...
        case KEY_ACTION_PAGE_TURN_TUNE: return "PAGE TURN";
        case KEY_ACTION_PAGE_SERVO_TUNE: return "PAGE SERVO";
        case KEY_ACTION_COORD_TASK:   return "COORD TASK";
...
static uint8_t driver_required_action(KeyAction_t action)
{
    return (action == KEY_ACTION_TEST_LOW ||
            ...
            action == KEY_ACTION_TEST_HEADING ||
            action == KEY_ACTION_TEST_AUTO ||
...
static void execute_action(KeyAction_t action)
{
    ...
    switch (action)
    {
        ...
        case KEY_ACTION_TEST_HEADING:
            /* Start locked-heading mode: keep current yaw, set forward speed to 0.
             * This runs closed-loop azimuth lock! Any manual rotation will be resisted.
             */
            Chassis_SetAbsoluteHeading(Chassis_GetYaw(), 0);
            set_event_text("RUN: heading lock");
            break;

        case KEY_ACTION_TEST_AUTO:
            /* Start closed-loop auto straight-line driving: lock current yaw, set speed dynamically.
             * This runs closed-loop speed PI AND azimuth lock together!
             */
            Chassis_SetAbsoluteHeading(Chassis_GetYaw(), g_auto_target_speed);
            set_event_text("RUN: closed auto");
            break;

        case KEY_ACTION_ZERO_YAW:
            Chassis_ZeroYaw();
            set_event_text("YAW: zeroed!");
            break;

        case KEY_ACTION_SAVE_PARAMS:
            Chassis_SaveParamsToFlash();
            set_event_text("SAVE: params saved!");
            break;
        ...
        case KEY_ACTION_PAGE_TURN_TUNE:
            Chassis_Stop();
            TurnTune_Init();
            DisplayUI_SetPage(UI_PAGE_TURN_TUNE);
            set_event_text("LCD: turn page, motor stop");
            break;

        case KEY_ACTION_PAGE_SERVO_TUNE:
            Chassis_Stop();
            ServoTune_Init();
            DisplayUI_SetPage(UI_PAGE_SERVO_TUNE);
            set_event_text("LCD: servo tune page");
            break;

        case KEY_ACTION_COORD_TASK:
            CoordinatedTask_Start();
            set_event_text("RUN: coord task");
            break;
...
void KeyControl_Task(void)
{
    ...
    /* ---- SERVO TUNE page: override key behavior for Servo tuning ---- */
    if (DisplayUI_GetPage() == UI_PAGE_SERVO_TUNE)
    {
        if (key0.short_event)
        {
            ServoTune_NextParam();
        ...
        return;   /* Skip normal key handling when on SERVO TUNE page. */
    }

    /* ---- TURN TUNE page: override key behavior for Turn PID tuning ---- */
    if (DisplayUI_GetPage() == UI_PAGE_TURN_TUNE)
    {
        if (key0.short_event)
        {
            TurnTune_NextParam();
        ...
        return;   /* Skip normal key handling when on TURN TUNE page. */
    }
