/**
 * @file display_ui.c
 * @brief LCD本地显示界面系统，使用差异化字符重绘机制实现无闪烁快速刷新仪表盘界面，展示电压、姿态、PID参数等信息。
 * @author 李帅 赵禹博 吴坨鑫
 * @date 6月12号
...
static void draw_static_layout(void)
{
    ...
    switch (g_page)
    {
        ...
        case UI_PAGE_SERVO_TUNE:
            draw_header("Gimbal Servo Tuning");
            ui_print(4, 32, "---- S1 (PAN) ---- ---- S2 (TILT) ---", GRAY);
            ui_print(4, 210, "K0: sel item   K1: adjust/run", LGRAY);
            break;
...
static char servo_tune_cursor(ServoParamIndex_t sel, ServoParamIndex_t me)
{
    return (sel == me) ? '>' : ' ';
}

static void update_servo_tune_page(void)
{
    const ServoTuneState_t *t = ServoTune_GetState();
    char buf[64];
    ServoParamIndex_t sel = t->selected;

    snprintf(buf, sizeof(buf), "%c1. MODE: %s", 
             servo_tune_cursor(sel, SERVO_PARAM_MODE), 
             servo_mode == 1 ? "REAL ACTIVE" : "VIRTUAL TEST");
    ui_value_line(48, buf, WHITE);

    snprintf(buf, sizeof(buf), "%c2. S1 Cent: %d us", 
             servo_tune_cursor(sel, SERVO_PARAM_S1_CENT), s1_center);
    ui_value_line(64, buf, WHITE);

    snprintf(buf, sizeof(buf), "%c3. S1 Ang : %d deg", 
             servo_tune_cursor(sel, SERVO_PARAM_S1_ANG), s1_angle);
    ui_value_line(80, buf, WHITE);

    snprintf(buf, sizeof(buf), "%c4. S2 Cent: %d us", 
             servo_tune_cursor(sel, SERVO_PARAM_S2_CENT), s2_center);
    ui_value_line(96, buf, WHITE);

    snprintf(buf, sizeof(buf), "%c5. S2 Ang : %d deg", 
             servo_tune_cursor(sel, SERVO_PARAM_S2_ANG), s2_angle);
    ui_value_line(112, buf, WHITE);

    snprintf(buf, sizeof(buf), "%c6. S1 Save: PRESS K1 TO SAVE", 
             servo_tune_cursor(sel, SERVO_PARAM_S1_SAVE));
    ui_value_line(128, buf, WHITE);

    snprintf(buf, sizeof(buf), "%c7. S2 Save: PRESS K1 TO SAVE", 
             servo_tune_cursor(sel, SERVO_PARAM_S2_SAVE));
    ui_value_line(144, buf, WHITE);

    snprintf(buf, sizeof(buf), "%c8. SWEEP  : PRESS K1 TO RUN", 
             servo_tune_cursor(sel, SERVO_PARAM_SWEEP));
    ui_value_line(160, buf, WHITE);

    if (limit_error == 1)
        snprintf(buf, sizeof(buf), "STATUS: OUT OF RANGE!");
    else if (limit_error == 2)
        snprintf(buf, sizeof(buf), "STATUS: S1 CENT SAVED!");
    else if (limit_error == 3)
        snprintf(buf, sizeof(buf), "STATUS: S2 CENT SAVED!");
    else if (limit_error == 4)
        snprintf(buf, sizeof(buf), "STATUS: SWEEP DONE!");
    else
        snprintf(buf, sizeof(buf), "STATUS: SAFE WORKING");

    ui_value_line(180, buf, limit_error == 1 ? RED : (limit_error ? GREEN : LGRAY));
}
